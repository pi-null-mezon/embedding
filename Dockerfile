...
RUN apt-get update && \
    ...
    pip3 install --no-cache-dir -r /usr/local/bin/requirements-cpu.txt && \
    pip3 install --no-cache-dir torch==1.12.0 torchvision==0.13.0 -f https://download.pytorch.org/whl/torch_stable.html
